...
FILE_PATH = 'challenge1_retail_churn - Datasource.xlsx'
...
with tab_fin:
    ...
    styled_cat_df = cat_df.style.format({'Current': '€{:,.2f}', 'Comparison': '€{:,.2f}', 'Δ%': '{:+.1f}%'}).applymap(
        color_delta,
        subset=['Δ%']
    )
    ...
    styled_reg_df = reg_df.style.format({'Current': '€{:,.2f}', 'Comparison': '€{:,.2f}', 'Δ%': '{:+.1f}%'}).applymap(
        color_delta,
        subset=['Δ%']
    )
...
with tab_segment:
    ...
    if len(scorecard) > 0:
        ...
        styled_df = scorecard_display.style.applymap(
    ...
    if len(migration_matrix) > 0:
        ...
        fig_heat = go.Figure(data=go.Heatmap(
            ...
            colorscale='Blues',
            showscale=True,
            colorbar=dict(title="% of<br>Segment", titleside='right', thickness=15, len=0.7),
