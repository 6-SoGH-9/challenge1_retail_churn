...
with tab_rfm:
    ...
    if len(rfm_data) == 0:
        ...
        with k1:
            st.metric("Customers", f"{filtered_rfm['customer_id'].nunique():,}")
        with k2:
            st.metric("Total Spend", f"€{filtered_rfm['Monetary'].sum():,.0f}")
        with k3:
            st.metric("Avg Frequency", f"{filtered_rfm['Frequency'].mean():.1f}")
        with k4:
            st.metric("Avg Recency", f"{filtered_rfm['Recency'].mean(skipna=True):.0f} days")
...
with tab_fin:
    ...
    with mt1:
        start_month_idx = st.selectbox(
            ...
            index=max(0, len(month_labels) - 24),
            key='mt_start_month'
        )
        start_month = pd.Timestamp(month_labels[start_month_idx])
    
    with mt2:
        end_month_idx = st.selectbox(
            "End Month",
            options=range(len(month_labels)),
            format_func=lambda i: month_labels[i],
        ...
        end_month = pd.Timestamp(month_labels[end_month_idx])
    ...
    if start_month > end_month:
        ...
        # Calculate comparison period if selected
        comparison_data = None
        if mt_comparison == 'Previous Period':
            period_length = (end_month - start_month).days
            comp_end = start_month - pd.Timedelta(days=1)
            comp_start = comp_end - pd.Timedelta(days=period_length)
        ...
        elif mt_comparison == 'Previous Year':
        ...
        # Only show deltas if comparison data exists and is not empty
        show_delta = comparison_data is not None and len(comparison_data) > 0
        
        with summary_cols[0]:
            st.metric(
                "Total Revenue",
                f"€{monthly_data['Revenue'].sum():,.0f}",
                delta=f"€{(comparison_data['Revenue'].sum() - monthly_data['Revenue'].sum()):,.0f}" if show_delta else None
            )
        ...
        with summary_cols[1]:
            st.metric(
                ...
                delta=f"{(comparison_data['Transactions'].sum() - monthly_data['Transactions'].sum()):,.0f}" if show_delta else None
            )
        
        with summary_cols[2]:
            st.metric(
                "Avg Monthly Customers",
                f"{monthly_data['Active_Customers'].mean():,.0f}",
                delta=f"{(comparison_data['Active_Customers'].mean() - monthly_data['Active_Customers'].mean()):,.0f}" if show_delta else None
            )
