...
def build_new_customer_forecast(forecast_months, acquisition_per_month, revenue_per_new_customer,
    ...
    forecast horizon, crediting revenue every month a cohort is active using
    the REAL observed monthly retention medians (M0, M1, M2, ... M12) -- no
    interpolation. retention_medians must already contain every month in
    range(0, max_horizon+1) (see calculate_cohort_assumption_ranges with
    milestones=tuple(range(0, 13))).

    Returns a DataFrame: one row per forecast acquisition month, with
    columns M0, M3, M6, M9, M12 showing retained customer counts at those
    checkpoints (capped to the forecast horizon, for display purposes), plus
    a 'Revenue' column with the total projected revenue for that cohort,
    summed across every active month in the horizon.
    """
    all_months = sorted(retention_medians.keys())
    max_month = max(all_months) if all_months else 0
    display_milestones = [m for m in (0, 3, 6, 9, 12) if m in retention_medians]
    rows = []
    ...
    for i in range(forecast_months):
        ...
        months_remaining_in_horizon = forecast_months - i  # includes month 0 (acquisition month itself)

        # Credit revenue every month the cohort is active within the horizon,
        # using the real observed monthly retention rate for that month
        # (falls back to the furthest known month's rate if the horizon
        # extends past the available retention data)
        cohort_revenue = 0.0
        for active_month in range(months_remaining_in_horizon):
            rate = retention_medians.get(active_month, retention_medians.get(max_month, 0.0))
            if pd.isna(rate):
                rate = retention_medians.get(max_month, 0.0)
            retained_this_month = acquisition_per_month * rate
            cohort_revenue += retained_this_month * revenue_per_new_customer

        # Display columns still show the milestone checkpoints (M0/M3/M6/M9/M12)
        # for readability in the cohort table
        for m in display_milestones:
            if m == 0:
                row['M0'] = round(acquisition_per_month)
            elif m <= months_remaining_in_horizon - 1:
...
with tab_rfm:
    ...
    if len(rfm_data) == 0:
        ...
        # Filter transactions by Region and Category
        trans_filtered = transactions[
            (transactions['store_region'].isin(rfm_regions)) &
            (transactions['category'].isin(rfm_categories))
        ]
        
        # Recalculate RFM metrics ONLY on filtered transactions
        # This ensures Recency, Frequency, Monetary reflect only the selected region/category
        rfm_filtered_calc = calculate_rfm_as_of_date(trans_filtered, cutoff_date)
        
        # Get customers from filtered RFM, then apply segment filter
        filtered_customer_ids = rfm_filtered_calc['customer_id'].unique() if len(rfm_filtered_calc) > 0 else []
        
        filtered_rfm = rfm_data[
            (rfm_data['Segment'].isin(selected_segments)) &
            (rfm_data['customer_id'].isin(filtered_customer_ids))
        ].copy()
        
        # Replace Recency, Frequency, Monetary with the filtered-based calculations
        # This ensures metrics match the selected region/category filters
        if len(rfm_filtered_calc) > 0 and len(filtered_rfm) > 0:
            recalc_metrics = rfm_filtered_calc[['customer_id', 'Recency', 'Frequency', 'Monetary']].set_index('customer_id')
            filtered_rfm_indexed = filtered_rfm.set_index('customer_id')
            filtered_rfm_indexed.update(recalc_metrics)
            filtered_rfm = filtered_rfm_indexed.reset_index()
...
with tab_fin:
    ...
    if start_month > end_month:
        ...
        # Add Previous Year first (left) if it exists
...
with tab_forecast:
    ...
    # Monthly milestones (M0..M12) so every in-between month (M1, M2, M4...M11) has its
    # own REAL observed median/min/max -- no interpolation between sparse checkpoints.
    # max_months_back is widened to 24 (rather than 12) because a cohort needs to be at
    # least 12 months old to have a real M12 data point -- with only 12 trailing cohorts,
    # none of them are old enough to have reached M12, so it always falls back to M11.
    # Widening the lookback pulls in older cohorts (13-24 months back) so M12 is populated too.
    ranges = calculate_cohort_assumption_ranges(transactions, max_months_back=24, milestones=tuple(range(0, 13)))
    retention_medians = ranges['retention_medians']
    retention_ranges = ranges['retention_ranges']

    # Reference milestone for the slider = the exact forecast horizon month (capped to
    # the max month we have retention data for), not a rounded-down checkpoint.
    available_milestones = [m for m in retention_medians if m > 0 and not np.isnan(retention_medians[m])]
    max_available_milestone = max(available_milestones, default=12)
    ref_milestone = min(horizon_months, max_available_milestone) if available_milestones else 3
    ...
    if retention_min >= retention_max:
        # Only one cohort has reached this milestone within the lookback window,
        # so min == max. Pad with a small buffer to keep the slider usable.
        pad = max(median_retention_rate * 0.1, 0.01)
        retention_min = max(0.0, retention_min - pad)
        retention_max = min(1.0, retention_max + pad)
    ...
    with s2:
        rev_min = round(ranges['revenue_min'], 2)
        rev_max = round(ranges['revenue_max'], 2)
        rev_default = round(ranges['revenue_median'], 2)
        if rev_min >= rev_max:
            pad = max(rev_default * 0.1, 0.5)
            rev_min = max(0.0, rev_min - pad)
            rev_max = rev_max + pad
        revenue_per_new_customer = st.slider(
            "Avg Revenue per New Customer (€)",
            min_value=rev_min,
            max_value=rev_max,
            value=min(max(rev_default, rev_min), rev_max),
            key='fc_revenue'
        )
    with s3:
    ...
    st.caption(
        "Retention medians (key months): " +
        ", ".join([f"M{m} = {retention_medians[m]*100:.1f}%" for m in sorted(set([ref_milestone, 1, 3, 6, 9, 12]))
                   if m in retention_medians and not np.isnan(retention_medians[m])])
    )